...
| 校验 | 失败拒因 |
|---|---|
| `platform` 在 `{android, ios, harmony}` 白名单 | `invalid_platform` |
| 对应 `platform` 当前至少存在一台 `online` 设备（不要求 ready） | `no_device_on_platform` |
| 若传了 `deviceAlias`，必须在 `device_aliases` 表命中 | `unknown_device_alias` |
| `caseId` / `runContent` 非空 | `missing_field` |
...
| 场景 | 行为 |
|---|---|
| 未传 `deviceAlias` | 平台级调度，池中择一执行 |
| 传了别名 · 准入阶段未命中 | **整批同步拒绝**（`unknown_device_alias`），不入队 |
| 传了别名 · 已准入但目标设备离线 / 未就绪 / 被占用 | 保持排队，**只等这一台**，不降级到池 |
| 传了别名 · 别名在运维侧被中途删除 | item 保持排队直到 submission 3h 超时，终结为 `submission_timeout`，消费方明确收到 |

严格指名是契约级承诺：调用方指定了哪台，执行与报告就落在哪台，不会被悄悄换人。
