...
> 断言系统兜底示例：VLM 自认 `finished`，但断言要求"中奖 200 元"，结算页只显示正确率 60% 无中奖信息 → 终局裁决器拒绝放行，落 `assert_failed`，避免误判成功。
>
> ![断言失败兜底 · 防止 VLM 自我满足，强制业务条件复核](./assets/screenshots/device-workbench-android-assert-fail.png)
