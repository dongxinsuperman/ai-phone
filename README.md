...
**6. 应用分发（三端通用）** —— 上传 APK / HAP / IPA 包（Android / HarmonyOS / iOS 三端），按平台筛出可分发设备，一键批量安装到设备池，实时回传每台安装结果、支持失败重试：
...
ai-phone 有两条同源、并行维护的架构线，底层区别在「VLM 决策执行脑放在哪」。两者**核心数据库 schema 与基础产品能力兼容**；但**新增大功能优先（或仅）落地 `main`，部分大功能（如 Android 虚拟机）为 `main` 独有、本分支不含**：
...
| **独有大功能** | ✅ Android 虚拟机（Emulator）等，新增大功能优先落地 | ⚠️ 暂不含 `main` 的新增大功能（如虚拟机） |
| 适合 | 多 Agent 分布执行、就近决策 | 合规要求 Agent 不持有模型出口、强集中管控 / 审计 |

> **怎么选**：默认用 `main`（也是新增大功能的落地线，如需 Android 虚拟机只能用 `main`，本分支不含）。仅当合规要求"Agent 不得持有模型能力、决策与密钥全集中在 Server"时才用本分支 `next/server-brain`。两条线**二选一部署**，数据库 schema 兼容（同一时间只让一个架构连同一套库跑）。
...
ai-phone 采用 **MIT License** 开源。Copyright (C) 2026 Dongxin and ai-phone contributors。官方默认主线是 `main`（分布式 Agent 大脑），新功能优先落地 `main`；本分支 `next/server-brain`（Server 大脑）作为并行架构线持续维护，但部分大功能（如 Android 虚拟机）为 `main` 独有、本分支不含。

协作上优先使用 Issues / Discussions 交流问题、场景和设计取舍。Pull Request 可以提交，但不承诺 review、响应时效或合并；PR 更适合作为 bug 报告、设计参考或候选补丁。

欢迎自由使用、修改、商用、分发、fork 后二次开发或长期维护自己的版本。基于本项目修改、分发或提供第三方版本时，请保留版权与 MIT License 声明，并保留必要的来源说明与第三方声明。
