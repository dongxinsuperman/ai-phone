...
**面向中小型公司的三端真机 AI 自动化中台** —— iOS / Android / HarmonyOS 同级原生支持，自然语言驱动的纯视觉决策，开箱即用的调度队列与多设备并发，执行器可插拔，一台 Mac 即可起完整链路。新 Mac 从 0 到 1 的完整部署见 [deployment-mac（三端新机部署指南）](./docs/deployment-mac（三端新机部署指南）.md)。
...
> 新 Mac 三端完备部署请看 [deployment-mac（三端新机部署指南）](./docs/deployment-mac（三端新机部署指南）.md)。
...
| [deployment-mac（三端新机部署指南）](./docs/deployment-mac（三端新机部署指南）.md) | 部署者 / AI 助手 | 新 Mac 从 clone 到 Android / iOS / HarmonyOS 三端可执行的完整步骤、终端清单、自检和故障处理 |
