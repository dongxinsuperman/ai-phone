...
>
> **请注意**：本分支不仅停止"新功能"开发，**日常的阈值微调、prompt 优化、稳定性改进、bug 修复等改动也一律在 [`next/server-brain`](../../tree/next/server-brain) 上进行，不会反向同步到本分支**。如果你正在做生产部署、长期维护、或对模型行为细节有要求，**强烈建议直接使用 `next/server-brain` 分支**，本分支仅适合作为 v0.1.x 历史快照或简化架构的学习参考。
>
