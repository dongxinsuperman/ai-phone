...
# ---- 常量（与 Groovy @Field 对齐，想调就这里改）----
SAFETY_MAX_STEPS = 100
CLICK_STUCK_THRESHOLD = 4
SCROLL_STUCK_THRESHOLD = 3
UNKNOWN_ACTION_STREAK_LIMIT = 3
CONSECUTIVE_SCREENSHOT_FAIL_LIMIT = 3
DEFAULT_WAIT_SECONDS = 3
# 单步 wait 上限。Groovy 老链路是 60s，但本系统的 case 经常包含"等视频播完"
# / "等服务端跑批"这类长等场景，60s 上限会被频繁裁剪、引发用户体感"等不动"。
# 1800s = 30 分钟，覆盖绝大多数业务场景；超过就明显是 VLM 走错路了，让裁剪
# 帮忙打点（_decide_wait_seconds 会标 clipped=True 并把 requested vs actual
# 都打到日志/事件里，便于复盘）。
MAX_WAIT_SECONDS = 1800
POST_ACTION_SETTLE_MS = 500
...
CHAIN_MAX_ACTIONS = 2          # 单轮决策最多串联多少个 Action（超出截断）
CHAIN_INNER_GAP_MS = 200       # 链内相邻两个 Action 的硬等间隔
...
# "走完末班车"，不能因为网络抖动卡死整个 Run，给一个偏宽松的 60s 超时；
# 超时会按 SKIP 处理（回退采纳主 VLM 结果），不会阻塞 Run 收尾。
ASSERTION_SYSTEM_TIMEOUT_SECONDS = 60.0
...
# 长的合法重试，又不至于让明显死循环拖太久。
STRUCT_CLICK_BUCKET_PX = 50            # 同坐标桶聚类容差（px，质心欧氏距）
STRUCT_CLICK_BUCKET_TRIGGER = 3        # 同桶累计 ≥ N 次 click → 召唤审判
STRUCT_SCREEN_REVISIT_HAMMING = 8      # pHash 汉明距 ≤ 8 视为同屏（256 bit ≈ 3%）
STRUCT_SCREEN_REVISIT_TRIGGER = 3      # 同屏访问 ≥ N 次 → 召唤审判
STRUCT_SCROLL_FLIP_WINDOW = 6          # 最近 N 次滚动里数方向翻转
STRUCT_SCROLL_FLIP_TRIGGER = 2         # 翻转 ≥ N 次 → 召唤审判（震荡 / 东找西找）
STRUCT_SCROLL_NOPROGRESS_DIFF = 0.02   # 前后帧 diff_rate ≤ 此值 = 页面几乎没动
STRUCT_SCROLL_NOPROGRESS_TRIGGER = 3   # 同方向连续 ≥ N 次无效滑动 → 召唤审判
...
#   - 30 (当前)：覆盖典型长 case 全程的合法重试需求；同时仍是"审判模型一直被
#     VLM thought 骗放行"的兜底（极端情况 30 次都没识破再硬砍）。第一道总闸是
#     MAX_WAIT_SECONDS=1800（30 分钟超时），ALLOW 上限是第二道。
STRUCT_AUDIT_ALLOW_LIMIT = 30
...
STRUCT_AUDIT_TIMEOUT_SECONDS = 30.0    # 审判调用单次超时；超时按 ALLOW 处理（不阻塞 Run）
...
#   - v2 (当前): 10 → 让 VLM 进入主流程后再开始巡检，前 5 步常是登录/进首页/
#     Tab 切换，绝大部分合法跳步集中在前期，10 步后 VLM 的推理已经稳定可监督
STRUCT_AUDIT_PERIODIC_INTERVAL = 30
PERIODIC_TRIGGER_PREFIX = "[周期巡检]"  # 周期 trigger 字符串前缀，区分 detector / periodic
...
STRUCT_KEYWORD_HARD_HIT = 2            # 四级标签命中数 ≥ 此值 → 直接结构化
STRUCT_STRICTNESS_HARD_SCORE = 5       # 严格度 ≥ 此值 → 直接结构化（强信号免审判）
STRUCT_STRICTNESS_AUDIT_SCORE = 3      # 严格度 ≥ 此值 → 借审判模型分类（中等信号）
...
class VLMRunner:
    ...
    def __init__(
        ...
        *,
        ...
        # VLM 客户端：懒构造（测试里可以注入 mock）
        system_prompt = build_system_prompt(self.goal)
        self.counter = TokenCounter()
        self.vlm = vlm_client or VLMClient(system_prompt, counter=self.counter)
    ...
    async def _main_loop(self) -> Tuple[bool, str]:
        ...
        for step in range(start_step, self.max_steps + 1):
            ...
            parsed_chain: List[A.ParsedAction] = [
                A.parse_action(s) for s in all_action_strs
            ]
    ...
    async def _supervisor_audit(
        ...
        prompt = (
            ...
            "  时，才能 KILL：\n"
            "    a) VLM 反复点击同一坐标桶——最近 N 步里 ≥ 3 次 click 落在同一区域\n"
            "       （坐标差距 < 50px），且页面没有任何推进；\n"
            "    b) VLM 进入了用例完全没有提及的入口/页面，且持续游离不回归；\n"
            "    c) VLM 在同一弹窗/错误页反复进出 ≥ 3 次仍无法跳出；\n"
    ...
    async def _chat_text(
        ...
        *,
        ...
        """辅助系统统一文本调用入口：跑一次 1.6 通用版（assistant_*）的 chat 对话。

        ② 通道判定（_classify_structured_via_supervisor）和 ③ 审判系统
        （_supervisor_audit）都通过本函数下行，所以模型一旦在 settings 里改了，
        两个调用方自动跟随。① 起跑线包名匹配走的是同模型同端点但 payload 略
        不同（自带"NONE"协议），保留独立函数 _match_package_name 处理。

        ``thinking_enabled``：是否走思考模式。豆包 1.6 是混合推理模型，开思考
        会从 ~1s 涨到 ~5-15s 但准确率显著提升。审判系统调用处显式传 True；
        通道判定 / 子步骤拆解这种轻任务保持 False。
        """
        ...
        if resp.status_code != 200:
            raise RuntimeError(f"{label} 请求失败: status={resp.status_code}")
        data = resp.json()
        text = str(data["choices"][0]["message"]["content"])
        self.counter.record(label, model, data.get("usage"))
        return text
        ...
        *,
        ...
        try:
            ...
            if isinstance(content, list):
                ...
                text = ""
            if not text:
                raise RuntimeError("Chat Completions 未返回可解析文本")
            self.counter.record("断言系统", model, data.get("usage"))
        except Exception as exc:  # noqa: BLE001
    ...
    async def _match_package_name(self, app_name: str, packages: List[str]) -> str:
        """用一次独立的 VLM 纯文本调用从包名清单里挑最匹配的一个。

        Groovy 原实现里是直接 HTTP，不走 vlmDecide 的上下文，这里保持同样做法
        避免污染主循环的 messages。
        """
        if not packages:
        ...
        # 包名匹配是单次纯文本，统一走辅助系统 1.6 通用版（assistant_model）。
        # 不和 _chat_text 复用是因为本调用有"返回 NONE 视为未匹配"的协议
        # 兜底，独立函数把这层兜底语义留在 open_app/close_app 路径里更清晰。
        # 字符串模糊匹配的轻任务，固定 disabled 不开思考——单次 ~1s 即可。
        payload: Dict[str, Any] = {
            "model": model,
            "temperature": 0,
            "top_p": 0,
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            resp = await client.post(api_url, json=payload, headers=headers)
        if resp.status_code != 200:
            raise RuntimeError(f"包名匹配请求失败: status={resp.status_code}")
        data = resp.json()
        target = str(data["choices"][0]["message"]["content"]).strip()
        self.counter.record("包名匹配", model, data.get("usage"))
