...
- 卡死检测：点击连续 N 次相同坐标 / 滚动连续 N 次同方向（N 由 env 控制，默认 click=2 / scroll=3）
...
# 模块常量名 → Settings 字段名。上面这些 Run 行为 / 卡死 / 审判阈值在 import 时从
# 本机 .env 固化；Distributed Agent Brain 下需要让它们随 Server 下发的配置生效。
_RUN_TUNING_CONST_TO_FIELD = {
    "SAFETY_MAX_STEPS": "run_max_steps",
    "MAX_WAIT_SECONDS": "run_max_wait_sec",
    "CLICK_STUCK_THRESHOLD": "click_stuck_threshold",
    "SCROLL_STUCK_THRESHOLD": "scroll_stuck_threshold",
    "UNKNOWN_ACTION_STREAK_LIMIT": "unknown_action_streak_limit",
    "CONSECUTIVE_SCREENSHOT_FAIL_LIMIT": "consecutive_screenshot_fail_limit",
    "CHAIN_MAX_ACTIONS": "chain_max_actions",
    "CHAIN_INNER_GAP_MS": "chain_inner_gap_ms",
    "ASSERTION_SYSTEM_TIMEOUT_SECONDS": "assertion_timeout_sec",
    "STRUCT_CLICK_BUCKET_PX": "audit_click_bucket_px",
    "STRUCT_CLICK_BUCKET_TRIGGER": "audit_click_bucket_trigger",
    "STRUCT_SCREEN_REVISIT_HAMMING": "audit_screen_revisit_hamming",
    "STRUCT_SCREEN_REVISIT_TRIGGER": "audit_screen_revisit_trigger",
    "STRUCT_SCROLL_FLIP_WINDOW": "audit_scroll_flip_window",
    "STRUCT_SCROLL_FLIP_TRIGGER": "audit_scroll_flip_trigger",
    "STRUCT_SCROLL_NOPROGRESS_DIFF": "audit_scroll_noprogress_diff",
...
def _refresh_run_tuning_from_settings() -> None:
    """把上面的模块级阈值常量刷新成"当前生效配置"的值。

    仅当 Agent 已应用 Server 下发配置（has_runtime_override 为真）时才刷新——
    生产 Agent 由此让业务调的阈值 env（经 Server 下发）在 VLM loop 内全部生效；
    测试环境无 override，模块常量保持 import 值 / monkeypatch 值，不受影响。

    可安全重复调用：配置全局一份，多设备并行 Run 刷新成同一份值，无串扰；
    模块内对这些常量的引用是运行时查 globals，刷新后自动读到新值，无需改动
    各处引用点。
    """
    try:
        from ai_phone.config import get_settings as _gs, has_runtime_override
    ...
    cfg = _gs()
    g = globals()
    for const_name, field_name in _RUN_TUNING_CONST_TO_FIELD.items():
...
class VLMRunner:
    ...
    def __init__(
        ...
        *,
        ...
        # Distributed Agent Brain：Run 启动前把 import 期固化的 Run 行为 / 卡死 /
        # 审判阈值常量刷新成 Server 下发配置的值（仅当已应用下发覆盖时生效；
        # 测试环境不触发，保持模块常量 / monkeypatch 值）。必须在读取这些常量前调用。
        _refresh_run_tuning_from_settings()
        self.run_id = run_id
        self.driver = driver
        self.goal = goal.strip()
        self._emit = emit
        self._stop_event = stop_event or asyncio.Event()
        # max_steps 未显式传入时取（刷新后的）安全上限，使 Server 下发的 run_max_steps
        # 生效；默认参数无法用刷新后的常量，故用 None 占位再在此解析。
        self.max_steps = max_steps if max_steps is not None else SAFETY_MAX_STEPS
        ...
        for step in range(start_step, self.max_steps + 1):
            ...
            await self._emit_event(
                make_event(
                    ...
                    # M4 片3b：随事件携带结构化动作（链首 type + parsed 链的 to_dict），供
                    # Agent 侧旁路收集器整理 V3 成品缓存。只丰富已有事件 payload，不改决策/
    ...
    def _format_action_history(self, limit: int) -> str:
        ...
        担心 token 失控？完全没必要：
          - STRUCT_AUDIT_HISTORY_LIMIT=100，覆盖默认单 Run 100 步硬上限
          - 极端情况下每步 thought 2000 字 → 总 ≈ 200K 字；不再对单条 thought
            做字符截断，避免切掉关键执行声明
    ...
    async def _extract_struct_substeps(self) -> Optional[str]:
        ...
        prompt = (
            ...
            "新增或遗漏原有动作内容。\n"
            "- 将所有子步骤去掉拆分时新增的编号后，按顺序拼接，必须与 goal "
            "中的操作步骤原文逐字一致。\n"
    ...
    async def _verify_finished_assertion(
        ...
        *,
        ...
        prompt 会指引模型按证据职责综合使用两张图、动作历史、thought 和
        finish_msg：最终图负责当前可见结果，历史负责不会持续显示的过程，动作
        前图只辅助判断最后一个动作的变化。两图相同本身不能直接判 FAIL。
    ...
    async def _wait_page_stable_for_vlm(self, step: int) -> StabilityResult:
        # Distributed Agent Brain：截图与稳定检测在 Agent 本地完成。
        # （历史 server_brain 下这里有一条经 RemoteDriver 的 wait_stable_screenshot_jpeg
        #  远程稳定检测分支，执行脑下沉后已移除——本地 driver 无该方法。）
