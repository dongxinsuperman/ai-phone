...
第一层 · 语义等价匹配（先做这一层，能 PASS 就 PASS，不要进入第二层挑刺）：
- 把「预期结果」中的每一条要求与附图 2 做语义对照，下列情况一律视为成立：
...
- 只要附图 2 中存在能让一个产品经理认可的"语义对应物"，该条预期结果即视为成立。
        ...
        *,
        ...
        if self._is_structured:
            return (
                ...
                "额外约束：\n"
                "- 如果主 VLM 的 thought / finished 内容与截图明显矛盾（如自述「已切换」但两图几乎相同），"
                "即使第一层语义匹配成立，也按第二层「页面归属客观事实矛盾」判 FAIL。\n"
