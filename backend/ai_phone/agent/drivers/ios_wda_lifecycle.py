...
``env_file=(".env.defaults", ".env", ".env.local")``——意味着项目默认策略和
部署把 ``AI_PHONE_IOS_WDA_LIFECYCLE_MODE=stable`` 放在 ``.env.local`` 都能被识别。
**禁止**直接 ``os.environ.get("AI_PHONE_IOS_WDA_LIFECYCLE_MODE")``：
``agent/main.py`` 顶部的 ``_load_dotenv()`` 只是兼容少数 os.environ 旁路，绕过 Settings 会
