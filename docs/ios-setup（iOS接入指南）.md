...
> iOS 走两条能力链路：WebDriverAgent（WDA，截图 / 镜像 / 触控 / 输入 / 已知 bundle id 启动）和 `pymobiledevice3` 设备服务（发现设备 / 查询应用列表 / 安装 / DVT 兜底）。
...
pip install -e ".[ios]"   # iOS 接入依赖；9.x 对 iOS 17+/26 更关键
```
...
日常 iOS 调试的基础常驻进程是 3 个：

| 终端 | 命令 | 作用 |
|---|---|---|
| A | `python -m ai_phone server --host 0.0.0.0 --port 8000` | 后端 Server |
| B | `python -m ai_phone agent` | 后端 Agent；stable 模式下不插线预热，进入工作台或跑任务时触发本次 USB 会话首次 WDA 启动 |
| C | `npm run dev` | 前端 |

`sudo pymobiledevice3 remote tunneld` 不是所有 iOS 版本的第四个必开终端。它只服务 `pymobiledevice3` 的 RSD / DVT / 部分设备服务通道：

- iOS 15 / 16：通常走 usbmux lockdown，不要求常驻 tunneld。
- iOS 17+ / 26：如果需要 RSD 设备服务、DVT 截图兜底、部分应用列表 / 进程控制能力，或日志出现 `tunneld 没有这个 udid`、RSD / DVT 相关错误，再常驻 tunneld。
- WDA 主控制链路不依赖 tunneld；点击、滑动、输入、WDA MJPEG、已知 bundle id 启动 App 走 WDA。
...
### 2.1 Run 语义里的 open_app 链路

手动点击、滑动、输入只需要 WDA 控制链路。自然语言 `open_app(app_name='洋葱')` 多一步“应用名解析”：

```text
open_app("洋葱")
-> 查询 iPhone 已安装 bundle id 列表
-> 辅助模型把“洋葱”匹配成具体 bundle id
-> WDA launch_app(bundle_id)
```

因此 `open_app` 同时依赖 WDA 和 `pymobiledevice3 installation_proxy`。当前实现不会再用 `ApplicationType=Any` 作为唯一入口，而是分开查询 `User` 和 `System` 后合并；一侧失败不会拖死另一侧。看到 `type=Any` 报错时，说明 Agent 版本落后，需要更新。
...
## 六、iOS 17+ / RSD 场景按需常驻 tunneld

```bash
# 按需常驻 tunneld（需要 sudo，不要 Ctrl-C）
...
> iOS 15 / 16 不需要为了基础控制固定开 tunneld。iOS 17+ / 26 如果走 DVT 兜底、RSD 设备服务或遇到 `tunneld 没有这个 udid` / `创建 DVT Screenshot 失败`，再保持该窗口常驻。
...
- `open_app("某个 App")` 需要查询应用列表；如果控制台点击/滑动正常但 Run 在 `iOS 列应用失败 type=User/System`，优先检查设备是否已解锁、是否信任此电脑、USB 线是否稳定；iOS 17+ 再检查 tunneld / RSD。
