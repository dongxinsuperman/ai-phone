  ...
  "items": [
    {
      ...
      "functionMapContext": "可选：仅当前 case 需要的功能入口或业务补充说明",
...
`functionMapContext` 是非必填的准确率增强输入：没有 Map 时，Goal、子步骤和截图仍构成
完整执行主链；提供 Map 时，表示调用方已为本次 Run 选择了相关业务执行上下文，模型会在
页面关系、具体对象、入口路径、测试数据、业务术语和异常处理上高权重参考，优先于自身常识
和无依据猜测。Map 不能改变任务、跨越子步骤或充当完成证据。正文仅在每个逻辑会话段首轮
User 中注入，不写入 System、RunLog、RunStep 或 HTML 报告。
