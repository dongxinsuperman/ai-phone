...
- Android 不再提供设备级自动上滑配置；实测标准链路是 wake 后用 `wm dismiss-keyguard` 收起无安全认证的 keyguard。
...
- `AI_PHONE_HARMONY_WAKE_SWIPE_ENABLED=true` 是能力总开关；实际哪些设备上滑由 Server DB / Web「设备配置」页维护。
