...
| 审判系统 | 本地异常探测器触发轻量模型，判断继续、修正或终止 | 辅助模型 token |
...
结构化任务启动时，辅助模型直接读取完整 goal，自行识别「操作步骤：」、
「[操作步骤]」、编号列表或连续动作段落，并输出一层连续编号清单。拆分综合参考
自然段、换行、原有编号、标点和动作语义，不把某一种具体符号设为必须切分的硬边界。

清单内容只允许切割操作步骤原文，不做优化、润色、概括或动作内容增删。Runner 不再
本地匹配步骤标题，也不二次拆分或编号；只做首行 `1. <子步骤>` 的轻量格式准入，随后
把同一份清单注入豆包、Claude 或 GPT 的 system prompt，贯穿整个 Run。会话分段重置
时复用这份 system prompt，不重新拆解。模型调用或格式异常时不注入额外清单，主 VLM
仍持有完整原始 goal。
...
最终断言不是只看最后一张图，也不是默认保守挑错，而是按证据职责综合：

- 原始 goal / runContent。
- 动作历史 / 缓存回放摘要：其中 Runtime 动作记录证明明确写出的调用状态，但不
  单独证明 UI 产生了预期业务结果；其中 thought 属于主 VLM 自述，不能作证。
- 最终截图：证明当前页面、控件、数字、选中态等直接可见结果。
- 最后一个动作前截图：只辅助判断最后一击是否产生了预期变化。
- 主 VLM 的 thought / finished：帮助理解完成声明，但不能单独证明完成。

最终截图中的直接可见事实与动作历史冲突时，以最终截图为准；最终截图没有展示
此前发生的过程，不等于该过程没有发生。before / after 两图相同也不会自动 FAIL；
但如果最后动作按任务语义必须产生可见变化，并且最终图没有对应目标状态，则可以据此
驳回，不依赖主 VLM 是否声称变化已经发生。
