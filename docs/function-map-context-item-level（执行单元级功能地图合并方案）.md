...
> 后续行为更新（2026-08）：本方案的数据接收、合并、落库与派发语义保持不变；
> Agent Prompt 层已把 Map 正文从 System 移到每个逻辑会话段的首轮 User。字段仍非必填，
> 但一旦提供，会在业务执行知识范围内高权重使用；System 只保留其权限边界。Map 原文
> 仍不进入查询、广播、webhook、RunLog、RunStep 或 HTML 报告。
