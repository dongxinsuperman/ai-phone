...
本文是 `next/server-brain` 当前对外集成契约。代码锚点：
...
| Server 大脑调试 | `/api/internal/server-brain/*` | Bearer | 仅排障使用 |
...
| `cacheMode` | 否 | 批次默认轨迹缓存模式；单 item 可覆盖 |
...
| `POST /api/internal/server-brain/driver-probe` | Server 大脑 RPC 排障 |
| `GET /api/internal/server-brain/state` | Driver RPC 等待器 / 线程池状态 |
