...
如果这台 Agent 接 iPhone，需要额外开一个终端常驻 `tunneld`。
...
这个终端不要关闭。Mac 重启后需要重新启动。

iPhone 每次重启、升级系统、首次接入新 Mac 后，再执行一次 DDI mount：
...
DDI mount 成功后，这个命令可以结束；`remote tunneld` 终端仍需常驻。
...
| iOS WDA 未就绪 | 检查 `remote tunneld` 是否常驻、DDI 是否 mount、`AI_PHONE_WDA_PROJECT_DIR` 是否正确、开发者 App 是否已信任 |
