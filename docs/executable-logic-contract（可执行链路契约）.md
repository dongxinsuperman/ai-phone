...
断言系统必须按证据职责裁决：最终截图证明当前可见状态；动作历史中的 Runtime 动作
记录只证明明确写出的调用状态，不单独证明 UI 业务结果；历史中的 thought 与最后的
thought / finished 同属主 VLM 自述，不能作证；最后动作前截图仅辅助判断最后一击变化。
日志或自述与最终截图的直接可见事实冲突时以最终截图为准；最终截图没有展示历史过程
不等于过程未发生；before / after 两图相同不能单独作为 FAIL 理由。

实时断言配置缺失、调用失败或输出协议异常时，Runtime 当前会明确记录 `SKIP`，并采纳
主 VLM 的 finished。该兜底必须显式保留在日志与文档中，不得静默更换终态。
