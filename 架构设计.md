...
| `submission_items` | `id(PK), submission_id(FK CASCADE), case_id, case_name, platform, run_content, device_alias, state, status_reason, run_id(nullable), device_serial(nullable), enqueued_at, started_at, finished_at`，`UNIQUE(submission_id, case_id, platform)` | item 级真相；`state ∈ queued / running / success / failed / cancelled`；`status_reason` 见 `scheduler.STATUS_REASONS` 11 项；`run_id` 在进 running 时回填 |
...
```json
{
  ...
  "items": [
    {"caseId": "...", "caseName": "...", "platform": "ios", "runContent": "打开设置 → WLAN", "deviceAlias": "ios-iphone14-01"}
  ]
}
```

- `runContent` 是最终可执行语义，**调用方自己闭环前置依赖**；执行器不做 case 树、不拼前置步骤、不做参数化
- `deviceAlias` 可选：传了就严格命中 `device_aliases` 表（查不到整批 400 拒绝），不传就在平台内挑任意 ready 设备
- 契约字段冻结见 [`对外调用清单.md`](./对外调用清单.md) v1.5
...
- ✅ P1 iOS WDA 自愈：断链 / 证书过期时自动重拉 xcodebuild 子进程，`-allowProvisioningUpdates` 顺带续签（详见 §9、codex后续计划表.md）
...
- ✅ 准入严格：外部传 `deviceAlias` 就必须命中本表，整批 400 拒绝；不传就平台内任意 ready 设备
